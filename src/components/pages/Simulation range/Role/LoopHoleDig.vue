<template>
  <div style="display: flex; flex-direction: column; width: 100%">
    <div>
      ...
      <el-input v-model="relateData" placeholder="请输入内容" style="width: 180px; margin-bottom: 10px"/>
    </div>
    <div>
      <!-- 新增 -->
      <el-button type="primary" @click="addDialogVisible = true">上传插件</el-button>
      <el-button type="primary" @click="queryPluginByOption" style="display: flex; float: right">查询</el-button>
      ...
      <div style="margin-top: 10px; display: flex; flex-direction: row; flex-wrap: wrap">
        <el-card class="card" v-for="(item, index) in datas" :key="index" style="width: 365px; margin: 10px">
          <template #header>
            {{ item.qemuPluginName }}
          </template>
          <div>
          ...
          <template #footer>
            <el-button @click="startDig(item)" style="width: 155px">开启挖掘</el-button>
            <el-button @click="deletePlugin(item)" style="width: 155px">删除</el-button>
...
const add = (formData) => {
  instance.post('/qemu/upload', formData, {
    headers: {
